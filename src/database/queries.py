...
class QueryBuilder:
            ...
            block = f"""
                (
                    SELECT 
                        ...
                        {target_pointer}.name AS target_name, 
                        {depth} AS hops,
    ...
    @staticmethod
    def _build_node_chain(steps: int, relationship_type: str = "relates_to") -> str:
        """
        ...
        Returns:
            Chain string like: <-relates_to.in<-relates_to.in
        """
        if steps == 0:
            return ""
        return "".join([f"<-{relationship_type}.in" for _ in range(steps)])

    @staticmethod
    def _build_target_pointer(depth: int, relationship_type: str) -> str:
        """Build the target pointer for a given depth."""
        return "in" + QueryBuilder._build_node_chain(depth - 1, relationship_type)
    ...
    def _build_triples_block(
        ...
        for step in range(depth):
            ...
            if step == 0:
                ...
                prev_node_chain = "in" + QueryBuilder._build_node_chain(step - 1, relationship_type)
                rel_ptr = f"{prev_node_chain}<-{relationship_type}"

            # Build triple components
            from_node = (
                "$parent.name" if step == 0 
                else f"in{QueryBuilder._build_node_chain(step - 1, relationship_type)}.name"
            )
            ...
            if step == depth - 1:
                to_node = f"{target_pointer}.name"
            elif step == 0:
                to_node = "in.name"
            else:
                to_node = f"{rel_ptr}.in.name"
